...
class RemoteCPLEXSolver:
    def __init__(self, fileName, localPath, config: dict, log=True, cplexTimeOut=60):
        ...
        self.cplex_executable = config["executable"]
        self.is_connect_by_pwd = config["is_connect_by_pwd"]
        ...
        if self.is_connect_by_pwd:
            ...
            self.password = None
            self.ssh_private_key_file = config["ssh_private_key_file"]

    def solve(self):
        # lpProblem.writeLP(self.lpFileName + '.lp')
        _, cplex_log = self.optimizeLPUsingRemoteCPLEX(
            self.localPath, self.remote_path, self.lpFileName
        )
        return cplex_log

    def optimizeLPUsingRemoteCPLEX(
        self, localPath="", remotePath="", lpBaseFileName="", sharableFolder=None
    ):
        success = True
        sshClient = None
        sftpClient = None
        ...
        try:
            sshClient = Helper.openSecureConnectionToRemoteServer(
                is_connect_by_pwd, server, username, password, ssh_private_key_file
            )
            remoteTransport = Helper.openSecureTransportToRemoteServer(
                is_connect_by_pwd, server, username, password, ssh_private_key_file
            )
            ...
            if not sharableFolder:
                success = Helper.putFileOnRemoteServer(
                    localFileName=localLPFileName,
                    remotePath=remoteLPFileName,
                    sftp=sftpClient,
                )
            ...
            try:
                ...
                if success:
                    success, cplex_log = Helper.executeCPLEXThroughSSH(
                        ...
                        remoteLPPath=remoteLPFileName,
                        remoteSolutionPath=remoteSolutionFileName,
                ...
                if not sharableFolder:
                    if success:
                        success = Helper.getFileFromRemoteServer(
                            localFileName=localSolutionFileName,
                            remotePath=remoteSolutionFileName,
                            sftp=sftpClient,
                        )
            ...
            logger.info(
                f"Exception encountered in optimizeLPUsingRemoteCPLEX. Exception is {exception}"
            )
            success = False
            try:
                sftpClient.close()
...
class Helper:
    @staticmethod
    def openSecureConnectionToRemoteServerByPrivateKey(
        server="", username="", privateKeyFile=""
    ):
    ...
    @staticmethod
    def openSecureConnectionToRemoteServerByPassword(
    ...
    @staticmethod
    def openSecureConnectionToRemoteServer(
        is_connect_by_pwd=False,
        server="",
        username="",
        password="",
        ...
        if is_connect_by_pwd:
            return Helper.openSecureConnectionToRemoteServerByPassword(
                server, username, password
            )
        else:
            return Helper.openSecureConnectionToRemoteServerByPrivateKey(
                server, username, ssh_private_key_file
            )

    @staticmethod
    def openSecureTransportToRemoteServerByPrivateKey(
        server="", username="", privateKeyFile=""
    ):
        private_key = paramiko.RSAKey.from_private_key_file(privateKeyFile)
        ...
        transport.packetizer.REKEY_PACKETS = pow(
            2, 40
        )  # 1TB max, this is a security degradation
        return transport

    @staticmethod
    def openSecureTransportToRemoteServerByPassword(
        server="", username="", password=""
    ):
        transport = paramiko.Transport((server, 22))
    ...
    @staticmethod
    def openSecureTransportToRemoteServer(
        is_connect_by_pwd=False,
        server="",
        username="",
        password="",
        ...
        if is_connect_by_pwd:
            return Helper.openSecureTransportToRemoteServerByPassword(
                server, username, password
            )
        else:
            return Helper.openSecureTransportToRemoteServerByPrivateKey(
                server, username, ssh_private_key_file
            )
    ...
    def putFileOnRemoteServer(localFileName="", remotePath="", sftp=None):
        ...
        try:
            ...
            logger.info(
                "Encountered error pushing LP file to Server. Local File:%s, remote file:%s, exception:%s"
    ...
    def getFileFromRemoteServer(localFileName="", remotePath="", sftp=None):
        ...
        try:
            ...
            if "No such file" not in exceptionMessage:
                logger.info(
                    f"Encountered error retriving solution file from remote Server. Local File: {localFileName}, remote file: {remotePath}, exception: {exceptionMessage}"
                )
                raise
        return success
    ...
    def getCPLEXTunningParameters(cplexTimeOut):
        ...
        cplex_tunning_cmds = Helper.getCPLEXTunningParametersCommon(
            cplexTimeOut, cplexThreads
        )
        return cplex_tunning_cmds

    @staticmethod
    def executeCPLEXThroughSSH(
        remoteLPPath,
        remoteSolutionPath,
        server="",
        username="",
        ...
        try:
            ...
            for line in data:
                if "CPLEX Error  1217: No solution exists" in line:
                    logger.info(
                        "executeCPLEXThroughSSH:CPLEX indicated no solution exist...."
                    )
                ...
                solution_line_match = match(
                    r"Solution time = (.*) sec\.  Iterations = (.*)", line
                )
                if solution_line_match:
                    cplex_log["solution_time"] = float(solution_line_match[1].strip())
                    cplex_log["iteration"] = int(
                        solution_line_match[2].strip().split(" ")[0]
                    )

                gap_line_match = match(
                    r"Current MIP best bound = (.*) \(gap = (.*), (.*)%\)", line
                )
                if gap_line_match:
    ...
    def getCPLEXTunningParametersCommon(cplexTimeOut, cplexThreads):
        ...
        cplex_tunning_cmds += (
            "set mip strategy startalgorithm 3" + "\n"
        )  # Network Simplex
